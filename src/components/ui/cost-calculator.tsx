...
const questions = [
  {
    ...
    type: 'radio',
    options: [
      { value: 'production', label: 'Производство', icon: Building2, color: 'from-blue-500 to-blue-600' },
      { value: 'trade', label: 'Торговля', icon: Warehouse, color: 'from-green-500 to-green-600' },
      { value: 'services', label: 'Услуги', icon: FileText, color: 'from-purple-500 to-purple-600' },
      { value: 'mixed', label: 'Смешанная', icon: TrendingUp, color: 'from-orange-500 to-orange-600' }
    ]
  },
  {
    id: 2,
    ...
    options: [
      ...
      { value: '51-200', label: '51-200 человек', icon: Users, color: 'from-purple-500 to-purple-600' },
  ...
  },
  {
    ...
    options: [
      ...
      { value: '6-20', label: '6-20 точек', icon: Warehouse, color: 'from-purple-500 to-purple-600' },
  ...
  },
  {
    ...
    options: [
      ...
      { value: 'basic', label: 'Базовая версия', icon: Server, color: 'from-yellow-500 to-yellow-600' },
      { value: 'standard', label: 'Стандартная', icon: Server, color: 'from-green-500 to-green-600' },
      { value: 'enterprise', label: 'Предприятие', icon: Server, color: 'from-blue-500 to-blue-600' }
    ]
  },
  {
    id: 5,
    ...
    type: 'checkbox',
    options: [
      { value: 'accounting', label: 'Проблемы с бухгалтерией', icon: FileText, color: 'from-red-500 to-red-600' },
      { value: 'production', label: 'Неэффективное производство', icon: Building2, color: 'from-orange-500 to-orange-600' },
      { value: 'warehouse', label: 'Хаос на складах', icon: Warehouse, color: 'from-yellow-500 to-yellow-600' },
      { value: 'reports', label: 'Неточные отчеты', icon: TrendingUp, color: 'from-purple-500 to-purple-600' },
      { value: 'integration', label: 'Проблемы с интеграциями', icon: Zap, color: 'from-blue-500 to-blue-600' },
      { value: 'performance', label: 'Медленная работа системы', icon: Clock, color: 'from-green-500 to-green-600' }
    ]
  },
  {
    id: 6,
...
const calculateCost = (answers: any) => {
  ...
  switch (answers.industry) {
    case 'production': baseCost = 800000; break;
    case 'trade': baseCost = 500000; break;
    case 'services': baseCost = 300000; break;
    case 'mixed': baseCost = 600000; break;
    default: baseCost = 500000;
  }
  
  // Множитель по количеству сотрудников
  switch (answers.employees) {
    case '1-10': baseCost *= 0.8; break;
    case '11-50': baseCost *= 1.0; break;
    case '51-200': baseCost *= 1.3; break;
    case '200+': baseCost *= 1.6; break;
  }
  
  // Множитель по количеству точек
  switch (answers.locations) {
    case '1': baseCost *= 0.9; break;
    case '2-5': baseCost *= 1.0; break;
    case '6-20': baseCost *= 1.2; break;
    case '20+': baseCost *= 1.4; break;
  }
  
  // Множитель по текущей системе
  switch (answers.currentSystem) {
    case 'none': baseCost *= 1.2; break;
    case 'basic': baseCost *= 1.1; break;
    case 'standard': baseCost *= 1.0; break;
    case 'enterprise': baseCost *= 0.9; break;
  }
            ...
            <div className="mb-8">
                ...
                >
                  {currentQuestion.options.map((option: any, index: number) => (
                    <div key={index} className="relative">
                      ...
                      >
                        <div className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${option.color} text-white flex items-center justify-center mb-4`}>
              ...
              {currentQuestion.type === 'checkbox' && currentQuestion.options && (
                <div className="grid gap-4 md:grid-cols-2">
                  {currentQuestion.options.map((option: any, index: number) => (
                    <div key={index} className="flex items-start space-x-3 p-4 border-2 border-gray-200 rounded-2xl hover:border-blue-300 hover:bg-blue-50 transition-all duration-200">
                      <Checkbox
                        id={`checkbox-${index}`}
                        ...
                        onCheckedChange={(checked) => {
                          ...
                          if (checked) {
                            ...
                            handleAnswer(currentStep, currentProblems.filter((p: string) => p !== option.value));
                          }
                        }}
                        className="mt-1"
                      />
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-r ${option.color} text-white flex items-center justify-center`}>
                          <option.icon className="h-5 w-5" />
                        </div>
                        <Label htmlFor={`checkbox-${index}`} className="text-base font-medium text-gray-900 cursor-pointer">
                          {option.label}
                        </Label>
                      </div>
                    </div>
